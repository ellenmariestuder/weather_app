import React, { useState } from 'react'
import Cities from './Cities'
import Modal from './Modal'

const Landing = ({ data_c1, data_c2, data_c3, data_c4, data_c5, data_c1_w, data_c2_w, data_c3_w, data_c4_w, data_c5_w }) => {
  const [data, setData] = useState();
  const [details, setDetails] = useState();
  const [show, setShow] = useState(false);

  const handleClick = (data, details) => {
    setData(data);
    setDetails(details);
    setShow(!show);

    console.log(data);
    console.log(details);
  }

  return (
    <div className="background">
      <div className="container">
        <h1>Weather</h1>

        <form>
          <img className="search-icon" src="data:image/svg+xml;utf8;base64," />
          <input type="text" name="name" placeholder="Search" />
          {/* <input type="submit" value="Go" /> */}
        </form>

        <Cities handleClick={handleClick} data_c1={data_c1} data_c2={data_c2} data_c3={data_c3} data_c4={data_c4} data_c5={data_c5} data_c1_w={data_c1_w} data_c2_w={data_c2_w} data_c3_w={data_c3_w} data_c4_w={data_c4_w} data_c5_w={data_c5_w} />
        <Modal show={show} setShow={setShow} data={data} details={details} />

        <div className="footer-text">
          Built with <a href="https://openweathermap.org" target="_blank">openweathermap.org</a> API
          by <a href="https://github.com/ellenmariestuder" target="_blank">@ellenmariestuder</a></div>
      </div>

    </div>
  )
}

export default Landing;